...
class GreenBlob(Blob):
        ...
        def __add__(self, other_blob):
            logging.info('Blob add up {} + {}'.format(str(self.color), str(other_blob.color)))
    ...
    for green_id, green_blob, in greens.copy().items():
        for other_blobs in blues, reds, greens:
            for other_blob_id , other_blob in other_blobs.copy().items():
                logging.debug('Checking if blobs are touching {} + {}'.format(str(green_blob.color), str(other_blob.color)))
